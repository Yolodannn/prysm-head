...
case "$MODE" in
  k2|k3|k4|no-attack)
    ;;
  *)
    echo "Usage: $0 {k2|k3|k4|no-attack}"
    exit 1
    ;;
esac
...
WORKTREE="${REORG_WORKTREE:-${SUBMISSION_ROOT}-reorg-${MODE}}"

# Generated chain data and outputs.
BASE="${BASE:-$SUBMISSION_ROOT/../reorg-artifacts/${MODE}}"

case "$MODE" in
  k2)
    K=2
    PATCH="$SUBMISSION_ROOT/experiments/reorg/k2/attack.patch"
    ENV_FILE="$SUBMISSION_ROOT/experiments/reorg/k2/333byz.env"
    ;;
  k3)
    K=3
    PATCH="$SUBMISSION_ROOT/experiments/reorg/k3/attack.patch"
    ENV_FILE="$SUBMISSION_ROOT/experiments/reorg/k3/333byz.env"
    ;;
  k4)
    K=4
    PATCH="$SUBMISSION_ROOT/experiments/reorg/k4/attack.patch"
    ENV_FILE="$SUBMISSION_ROOT/experiments/reorg/k4/333byz.env"
    ;;
  no-attack)
    K=""
    PATCH="$SUBMISSION_ROOT/experiments/reorg/k4/attack.patch"
    ENV_FILE="$SUBMISSION_ROOT/experiments/reorg/no-attack/333byz.env"
    ;;
esac
...
if [[ "$MODE" != "no-attack" ]]; then
  ...
  unset EXPERIMENT_REORG_K
  unset EXPERIMENT_REORG_EVENTS_PATH
  unset EXPERIMENT_REORG_WINDOWS_FILE
  unset EXPERIMENT_REORG_RESULTS_CSV
  unset EXPERIMENT_REORG_PRIVATE_ROOTS_CSV
fi
...
if [[ "$MODE" == "no-attack" ]]; then
  ...
  # Control experiment
  #
  # EXPERIMENT_ATTACK_MODE=none disables the reorg-specific
  # role filter. Therefore the two validator clients must use
  # disjoint key ranges to avoid duplicate duties.
  # ----------------------------------------------------------

  echo "Starting no-attack Byzantine/control group: validators 0-332"
  ...
  echo "Starting no-attack honest group: validators 333-999"
